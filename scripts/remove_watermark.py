...
Simple, effective: applies uniform correction to ALL pixels.
...
def remove_watermark(arr: np.ndarray, alpha: float = 0.01) -> np.ndarray:
    """Apply reverse alpha blending to entire image."""
...
def main():
    ...
    print(f"OK: {img.size[0]}x{img.size[1]} | alpha={args.alpha} | {orig_kb:.0f}KB -> {clean_kb:.0f}KB | -> {out}")
